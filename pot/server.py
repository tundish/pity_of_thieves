...
async def janitor(app):
    ...
    while True:
        ...
        orphans = [
            ...
            if (now - i.ts) > limit and i.story.turns < 4
        ]
        for i in orphans:
            log.info("Discard {0.story.id} after turn {0.story.turns}".format(i))
...
async def get_session(request):
    ...
    rv = story.render_body_html(title=title, next_=refresh_target, refresh=refresh).format(
        '<link rel="stylesheet" href="/css/theme/pot.css" />',
...
def run():
    ...
    rv = 0
    if args.version:
        sys.stdout.write(pot.__version__)
        sys.stdout.write("\n")
    else:
        rv = main(args)
